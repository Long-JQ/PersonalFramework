...
namespace PersonalFramework.Service
{
    public class UploadPic : System.Web.UI.Page
    {
        ...
        public static string MvcUpload(HttpPostedFileBase myFileUpload, string[] allowExtensions, int maxLength, string savePath, string addName="")
        {
            // 文件格式是否允许上传
            ...
            if (myFileUpload != null)
            {
                ...
                if (fileAllow)
                {
                    ...
                    string saveName = addName + "_" + Guid.NewGuid() + fileExtension;
                    ...
                    if (!PersonalFramework.Service.FileValidation.IsImage(path))
                    {
                    ...
                    if (!PersonalFramework.Service.FileValidation.IsPicture(path))
                    {
                        System.IO.File.Delete(path); //删除原文件
                        throw new Exception("您上传的不是图片，如果你上传多次不是图片的文件，系统将识别为木马文件，系统将自动冻结您的账号");
                    }
